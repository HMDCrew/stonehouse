...
const add_layers = (map, miniature) => {
    ...
    const big_map = L.tileLayer(defaults_maps.topografica, {
    ...
    L.tileLayer(defaults_maps.roards, {
        subdomains: 'abcd',
        maxZoom: 18
    }).addTo(map)
    ...
    return {
        map: big_map,
        mini: L.tileLayer(defaults_maps.geografica).addTo(miniature),
    }
}
...
const popup_marker_content = (map, save_location, latlng) => {
    ...
    const button = reder_el( 'button', ['btn', 'btn-add-house'], defaults_maps.save_point_map )
    ...
    button.addEventListener('click', ev => {
        ...
        new Promise( ( resolve ) => resolve( save_location( latlng ) ) ).then(
            () => {
                ...
                L.marker(latlng, { icon: defaults_maps.saved_marker }).addTo(map)
            }
        )
    }, false)
...
const set_marker = (map, latlng, popup_event) => {
    ...
    position_marker = L.marker(latlng, { icon: defaults_maps.marker }).addTo(map)

    point_marker ? point_marker.remove() : ''
    point_marker = L.marker( latlng, { icon: defaults_maps.point_marker }).addTo(map)
...
const switch_maps = (layers) => {
    if (defaults_maps.selected == "topografica") {
        layers.mini.setUrl(defaults_maps.topografica)
        layers.map.setUrl(defaults_maps.geografica)
        defaults_maps.selected = "geografica"
    } else {
        layers.mini.setUrl(defaults_maps.geografica)
        layers.map.setUrl(defaults_maps.topografica)
        defaults_maps.selected = "topografica"
    }
}
...
export const init_map = (init, save_location) => {
    ...
    const map = L.map('stonemap', {
        center: L.latLng(init.latitude || 0, init.longitude || 0),
        zoom: 14,
        minZoom: 3,
    })

    const miniature = L.map('mini-map', {
        center: L.latLng(init.latitude || 0, init.longitude || 0),
        zoom: 11,
        zoomControl: false,
